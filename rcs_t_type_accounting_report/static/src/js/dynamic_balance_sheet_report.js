...
export class BalanceSheetReport extends Component {
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            if (mainReportOptions.bl_sheet_companies && mainReportOptions.bl_sheet_companies.length > 0) {
                this.selected_branches = mainReportOptions.bl_sheet_companies.map(id =>
                    this.branch_list.find(branch => branch.id == id)
                ).filter(Boolean);
            }
            const selectBox = $('#many2many-select');
            if (selectBox) {
                $(selectBox).on("change", this.onBranchSelectBalance.bind(this));
            }
    ...
    async onWillStart() {
        this.branch_list = await this.orm.call('res.company', 'search_read', [
            [
                ['id', 'in', this.currentCompany.activeCompanyIds]
            ],
            ['name']
        ], {
    ...
    async loadData(initial_render = true) {
        ...
        if (save_options.bl_sheet_companies) {
            $('#many2many-select').val(save_options.bl_sheet_companies);
        } else {
